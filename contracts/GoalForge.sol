...
interface IGoalForgeNFT {
    enum HabitType { CODING, DSA, GYM, YOGA, RUNNING }
    function mint(address to, HabitType habitType, uint256 daysCompleted) external returns (uint256);
}
...
contract GoalForge is ERC20, Ownable {
    ...
    IGoalForgeNFT public nftContract;
    ...
    uint256 public constant BONUS_RATE = 10; // 10% bonus on successful completion
    ...
    constructor(address _nftContract) 
        ...
        Ownable(msg.sender) 
    {
        nftContract = IGoalForgeNFT(_nftContract);
    ...
    function completeGoal(address _user, uint256 _goalIndex) internal {
        ...
        uint256 stakeAmount = goal.stake;
        uint256 bonus = (stakeAmount * BONUS_RATE) / 100;

        // Return stake
        _transfer(address(this), _user, stakeAmount);
        userStakes[_user] -= stakeAmount;

        // Mint bonus reward tokens
        _mint(_user, bonus);

        // Mint NFT reward using the NFT contract
        uint256 tokenId = nftContract.mint(
            _user, 
            IGoalForgeNFT.HabitType(uint8(goal.habitType)), 
            goal.totalDays
        );
        userNFTs[_user].push(tokenId);
        
        emit GoalCompleted(_user, _goalIndex, stakeAmount, bonus);
    ...
    function failGoal(address _user, uint256 _goalIndex) internal {
        ...
        // Burn the stake
        _burn(address(this), stakeAmount);
        userStakes[_user] -= stakeAmount;

        emit GoalFailed(_user, _goalIndex, stakeAmount);
    }
